...
import { confirmSale } from "@/lib/sale-helpers-sprint4";
import { z } from "zod";
...
export async function POST(
  ...
  try {
    ...
    // SPRINT 4: Usar helper para confirmar venta
    const result = await confirmSale(params.id, tenantId);
    
    if (!result.success) {
      return errorResponse(result.error || "Error al confirmar la venta", 400);
    }
    ...
    if (fetchError || !saleWithItems) {
      ...
      return errorResponse("Error al obtener la venta confirmada", 500, fetchError?.message);
    }
