...
import { getAllAlerts } from "@/lib/alert-helpers";
import { z } from "zod";

// SPRINT H: GET /api/reports/alerts - Endpoint unificado de alertas
export async function GET(req: Request) {
  try {
    ...
    const alerts = await getAllAlerts(tenantId, lowMarginThreshold, startDate, endDate);

    return jsonResponse(alerts, 200);
  } catch (error) {
    return handleUnexpectedError(error, "GET /api/reports/alerts");
  }
}
